...
- **Schema not in version control.** Per `replit.md`, most DB/RLS changes are applied live via `supabase db query --linked` against the staging/production Supabase projects and are never captured as `.sql` files here. The `.sql` files in `supabase/` are a partial, dated trail — not a source of truth. Verify against the live Supabase dashboard before assuming this repo reflects the real schema.
